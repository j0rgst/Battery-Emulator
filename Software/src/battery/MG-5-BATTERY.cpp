...
void Mg5Battery::handle_incoming_can_frame(CAN_frame rx_frame) {
  ...
  switch (rx_frame.ID) {
    ...
    case 0x620:
      break;
    default: {  // response from UDS diagnostic service (ISO-TP)

      if(rx_frame.ID == uds_address+8) { // only process if message is from the current UDS address we are talking to
